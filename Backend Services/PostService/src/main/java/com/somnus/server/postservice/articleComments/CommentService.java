...
import com.somnus.server.postservice.articleComments.repository.CommentRepository;
...
import com.somnus.server.postservice.notifications.handlers.NotificationHandler;
...
//    private UserRepository userRepository;
    ...
    public CommentDto addCommentToArticle(User user, CommentDto commentDto) {
        ...
        Comment comment = new Comment(commentDto.getArticleId(), user.getFirebaseGeneratedUserID(),
...
//                resultComment);
    ...
    public CommentDto updateComment(User user, CommentDto commentDto) {
        ...
        if (!comment.getUsername().equals(user.getFirebaseGeneratedUserID())) {
    ...
    public void addCommentLike(User user, Integer commentId) {
        ...
        // Possible search optimization needed in the future
        boolean userLike = comment.getUserLikes().contains(user.getId());
        if (!userLike) {
            comment.setNumLikes(comment.getNumLikes() + 1);
            comment.addUserLikes(user.getId());
            commentRepository.save(comment);
        }
    }
    ...
    public void removeCommentLike(User user, Integer commentId) {
        ...
        if (userLike) {
            comment.setNumLikes(comment.getNumLikes() - 1);
            comment.removeUserLike(user.getId());
            commentRepository.save(comment);
        }
    }
    ...
    public void removeComment(User user, int commentId) {
        ...
        Comment comment = optionalComment.get();
        if(!comment.getUsername().equals(user.getFirebaseGeneratedUserID()) && !user.getRole().equals(Role.ADMIN)) {
